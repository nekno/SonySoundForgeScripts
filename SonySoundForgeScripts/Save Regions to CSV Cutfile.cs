/* =======================================================================================================
 *	Script Name: Save Regions to CSV Cutfile
 ...
 *	
 *  Note: The CSV file will be of the format IDENT(Int64),NAME(string),START_SECONDS(double),END_SECONDS(double)
 *  
 *  Note: The generated CSV file can be used as a cutfile for another app, e.g., FFMPEG.
 *  The start and end are represented in seconds (and microseconds), taking the sample position for the start and end (start + length) 
 *  and dividing by the sample rate.
 *
...
public class EntryPoint
{
	public string Begin(IScriptableApp app) {
		...
		foreach (SfAudioMarker mk in file.Markers)
		{
			...
			double regionStartInSeconds = file.PositionToSeconds(mk.Start);
			double regionEndInSeconds = file.PositionToSeconds(mk.Start + mk.Length);

			markers.Add(String.Join(",", new string[] { mk.Ident.ToString(), quoted(mk.Name), regionStartInSeconds.ToString(), regionEndInSeconds.ToString() }));
